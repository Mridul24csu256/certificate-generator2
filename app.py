...
if template_file:
    ...
    if data_file:
        ...
        # Font library (can add .ttf files in repo or system fonts)
        fonts = {
            "Arial": "arial.ttf",
        ...
        # ================= Generate & Download All Certificates =================
        if st.button("🚀 Generate All Certificates"):
            ...
            with zipfile.ZipFile(zip_buffer, "w") as zipf:
                for idx, row in df.iterrows():
                    ...
                    cert_filename = f"{row[selected_columns[0]]}_certificate.jpg"
                    img_bytes = BytesIO()
                    cert.save(img_bytes, format="JPEG")
                    zipf.writestr(cert_filename, img_bytes.getvalue())
